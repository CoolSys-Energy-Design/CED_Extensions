...
| [ ] | H3 | P1 | Open | Feeder lookup uses global `revit.doc` rather than the circuit’s document. | Resolve through `self.circuit.Document`; test non-active-document construction if a supported caller can produce it. |
...
| [ ] | H10 | P2 | Open | Tag Homeruns regenerates once per tag. | Design a two-phase placement/refinement flow while retaining per-wire rollback or equivalent failure isolation. |
...
| [ ] | M3 | P1 | Open | `select_filter()` returns the last sorted token instead of the newly created token. | Return `new_token` and add an identity assertion to the test. |
...
| [ ] | M5 | P1 | Open | `IsViewValidForElementIteration` is called without the required `Document`. | Pass `(doc, view.Id)` and narrow compatibility exception handling. |
...
| [ ] | M11 | P3 | Review | `theme_audit.py` contains a developer-specific path, but it is not imported by runtime code. The claimed missing DarkAlt file exists. | Move or make the developer audit portable when convenient; do not treat it as a production runtime failure. |
