...
class RevitLinkLightSource(object):
    ...
    def __init__(self, host_doc, link_instance):
...
def collect_host_fixture_options(doc):
    symbols = (
...
class LightMappingWindow(CEDWindowBase):
    ...
    def __init__(self, doc):
        ...
        self._placement_engine = PlacementEngine(doc)
    ...
    def _browse_csv_clicked(self, sender, args):
        csv_path = forms.pick_file(file_ext="csv", title="Select Lighting Fixture CSV")
        if not csv_path:
            return
    ...
    def _load_link_source(self):
        ...
        source = RevitLinkLightSource(self.doc, selected.link_instance)
        result = source.collect()
        self._apply_source_result(result)
        self._restore_saved_mappings()
        self._apply_auto_match(overwrite_existing=False)
