# -*- coding: utf-8 -*-
"""Create Circuits by Device Parameter - pure logic (no Revit API, no .NET).
...
# group. Matching is case-insensitive so values such as "Dedicated", "DEDICATED"
# and "AHU - dedicated" all have the same meaning.
DEDICATED_KEYWORD = "DEDICATED"


def is_dedicated_group_name(value):
    """Return True when a group name contains the dedicated-circuit keyword."""
    return DEDICATED_KEYWORD in str(value or "").upper()
...
def build_group_plan(group_key, load_name, panel, rating, members,
    ...
    when there are members, a selected panel, no voltage/pole mismatch, and a
    valid rating.
    """
    amps, rating_valid, rating_standard = parse_rating(rating)
    problems = list(validate_members(members))
    if not (panel or "").strip():
        problems.append("Panel is not selected")
    if not rating_valid:
        problems.append("Invalid breaker rating")
    poles = sorted(set(
        int(m.poles_value) for m in members
        if getattr(m, "poles_value", None)
    ))
    volts = sorted(set(
        m.voltage_key for m in members
        if getattr(m, "voltage_key", None) is not None
    ))
    return {
        "group_key": group_key,
        "load_name": (load_name or "").strip() or group_key,
        "panel": (panel or "").strip(),
        "schedule_notes": (schedule_notes or "").strip(),
        "rating_amps": amps,
        "rating_valid": rating_valid,
        "rating_standard": rating_standard,
        "poles": poles[0] if len(poles) == 1 else None,
        "voltage": volts[0] if len(volts) == 1 else None,
