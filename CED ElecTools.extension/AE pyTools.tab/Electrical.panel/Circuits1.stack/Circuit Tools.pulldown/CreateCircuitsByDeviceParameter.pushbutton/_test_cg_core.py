...
def run():
    ...
    plan = cg_core.build_group_plan("R5", "x", "", "20", [Row(1, 1, 120)])
    check("plan blocked without panel",
          not plan["ready"] and "Panel is not selected" in plan["problems"])

    check("new group key", cg_core.next_new_group_key(["NEW-1", "R1"]) == "NEW-2")

    # --- group-by parameter discovery ---
    rows_data = [
        {"group_values": {"Identity Mark": "RA-1", "Level": "L1", "Only_A": "x"}},
        {"group_values": {"Identity Mark": "RA-2", "Level": "L1", "Only_B": "y"}},
    ]
    opts = cg_core.common_group_params(rows_data)
    check("common params intersect", "Identity Mark" in opts and "Level" in opts)
    check("family type is display only", "Family:Type" not in opts)
    check("common params exclude non-shared", "Only_A" not in opts and "Only_B" not in opts)
    check("preferred ordering", opts.index("Identity Mark") < opts.index("Level"))
    check("empty rows -> no params", cg_core.common_group_params([]) == [])
    check("dedicated keyword is case insensitive",
          cg_core.is_dedicated_group_name("AHU dedicated") and
          cg_core.is_dedicated_group_name("DEDICATED") and
          cg_core.is_dedicated_group_name("fan - DeDiCaTeD"))
    check("non-dedicated name is ordinary",
          not cg_core.is_dedicated_group_name("AHU - NORMAL"))
    ...
    check("default first when no preferred",
           cg_core.default_group_param(["Level", "Other"]) == "Level")
    ...
    check("name default first when no preferred",
           cg_core.default_name_param(["Level", "Other"]) == "Level")
