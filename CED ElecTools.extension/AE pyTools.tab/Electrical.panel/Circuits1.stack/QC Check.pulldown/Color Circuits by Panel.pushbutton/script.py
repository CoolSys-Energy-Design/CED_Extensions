...
def create_or_update_legend_drafting_view(legend_view, color_map, template, fr_type):
    ...
    total_items = len(color_map)
    for idx, (panel_name, rgb) in enumerate(color_map):
        x, y = 0.0, (total_items - 1 - idx) * spacing
...
def main():
    ...
    with DB.TransactionGroup(doc, "Color Circuits by Panel") as tg:
        ...
        with DB.Transaction(doc, "Create/Update Filters") as tx1:
            ...
            # --- Special "No Panel" filter FIRST for top priority ---
        ...
        with DB.Transaction(doc, "Legend Creation2") as tx3:
            ...
            if not fr_type:
                ...
                return
            # Sort color_map alphabetically, but keep "__NO PANEL__" at the top
            sorted_items = []
            # Add NO_PANEL_FLAG at the beginning if it exists
            if NO_PANEL_FLAG in color_map:
                sorted_items.append((NO_PANEL_FLAG, color_map[NO_PANEL_FLAG]))
            # Then add all other panels sorted alphabetically
            sorted_items.extend(sorted(
                [(k, v) for k, v in color_map.items() if k != NO_PANEL_FLAG],
                key=lambda x: x[0].lower()
            ))
            
            create_or_update_legend_drafting_view(legend_view, sorted_items, template, fr_type)
